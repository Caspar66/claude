...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  const { id } = req.query;
  const occupationId = Array.isArray(id) ? id[0] : id;
  if (!occupationId) {
    res.status(400).json({ error: 'Missing occupation id' });
    return;
  }

  const url = `${BASE_URL}/occupations/${encodeURIComponent(occupationId)}/mappings`;
