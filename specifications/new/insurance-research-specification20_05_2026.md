# Insurance Research & Comparison Tool — Insurance Research Specification (20/05/2026)
...
* Default values for Global Options and Commissions are seeded from Workspace Preferences when a new scenario is created

**Global Options Tab:**
* Premium Projection Duration
  * Mandatory, Dropdown
  * Options: 3 Years, 5 Years, 10 Years, 15 Years, 20 Years
  * Default: from Workspace Preferences (system default: 15 Years)
  * Controls the number of years used for premium projection calculations
* Indexation
  * Mandatory, Number input with % suffix
  * Range: 0 to 100 (step 0.01)
  * Default: from Workspace Preferences (system default: 0)
  * Annual indexation rate applied to premiums
* Approved Product List
  * Mandatory, Dropdown
  * Options: Adviser, User
  * Default: from Workspace Preferences (system default: Adviser)
  * Determines which APL is used for quoting
* Minimum Commission Preference
  * Mandatory, Dropdown
  * Options: Yes, No
  * Default: from Workspace Preferences (system default: No)
  * When set to Yes, all providers are automatically assigned their minimum (0%) commission structure
  * A warning banner is displayed on the Commissions tab when enabled
...
* Commission Structure
  ...
  * Options: populated dynamically from the Suppliers API (each provider's `commissionOptions`)
  * Options display format: "[Structure] ([Upfront]% / [Ongoing]%): [Name]"
  * Default: seeded from Workspace Preferences commissions; falls back to each provider's default commission code from the API
* When Minimum Commission Preference is enabled:
  * All dropdowns are disabled (greyed out)
  * All providers are set to their minimum commission code (0% upfront / 0% ongoing)
  * An amber banner displays: "Minimum Commission Preference is enabled — all providers set to minimum (0%) commission."
...
### 2.11 Advisers can access supplier documents so that they can review PDS, adviser guides, and TMD documents
...
* **Table Columns:**
  ...
  * Date Issued (formatted DD/MM/YYYY, sortable — only shown for "Documents" category)
  * Description (document description, sortable)
  * Link (external link icon opening the document URL in a new tab)

* **Sorting:** Clicking a column header toggles ascending/descending sort; default sort by supplier ascending
* **Pagination:**
  * Page size selector: 10, 25, or 50 items per page
  * Page navigation with Previous/Next buttons and page number display
...
* Displayed below the results table inside the same scrollable container, separated by a top border
...
* The page displays a header bar (slate-700 background) with breadcrumb "Insurance Research > Scenarios > [Scenario Name]" and a back button
...
* **Top-Level Review Table:**
  * Products are displayed in a table with columns:
    ...
    * Super (premium amount with frequency suffix, e.g. "$123.45/month"; "N/A" if zero)
    * Non-Super (premium amount with frequency suffix; "N/A" if zero)
    * Premium (total premium amount with frequency suffix; uses non-super frequency when both differ, otherwise the common frequency)
...
* **Expandable Cover Details Sub-Table:**
  ...
  * **Type:** Cover type label (Life, TPD, Trauma, Income Protection, Business Expense, Other) derived via `getCoverType()` from the cover's need code
  * **Description:** Cover definition field (e.g. "Any", "Own", "Indemnity", "Agreed Value")
  * **Premium Structure:** Resolved from premium style code (S = Variable age-stepped, B = Blended, L = Variable to age 65, 70 = Variable to age 70)
  * **Cover Structure:** Derived using cover-type-specific rules (see Cover Structure Derivation below)
  * **Super:** Read-only checkbox, checked when the cover's `isSuper` flag is true
  * **Owner:** Editable dropdown with options: Client, Partner, SMSF, Super Fund, any custom owners, and "+ Add..." option
  * **Life Insured:** Name of the life insured (client or partner)
  * **Benefit Amount:** Sum insured formatted as currency
  * **Benefit Period:** Formatted as "X years" (<=5) or "Age X" (>5)
  * **Waiting Period:** Formatted as "X days"

* **Cover Structure Derivation for Existing Covers:**
  * **TPD covers:**
    * If superLinked = Yes → "Super-Linked"
    * If standAlone = Yes → "Standalone"
    * If flexiLinked = Yes → "Flexi-linked"
    * Otherwise → "Linked"
  * **Trauma covers:**
    * If no Life cover exists in the policy → "Standalone"
    * If standAlone = Yes → "Standalone"
    * If flexiLinked = Yes → "Flexi-linked"
    * Otherwise → "Linked"
  * **All other cover types (Life, IP, etc.):**
    * If superLinked = Yes → "Super-Linked"
    * If standAlone = Yes → "Standalone"
    * If flexiLinked = Yes → "Linked"
    * Otherwise → "Standalone"

* **Cover Structure for Recommended/Quote Covers:**
  * Linked extensions → "Linked"
  * Standalone covers → "Standalone"
  * SuperLink owner TPD → Two records created: (1) Super=True, Definition=Any, Cover Structure=Super-Linked; (2) Super=False, Definition=Own, Cover Structure=Super-Linked
  * SuperLink owner IP → Two records created: (1) Super=True, Cover Structure=Super-Linked; (2) Super=False, Cover Structure=Super-Linked

* **Owner Management:**
  * Owners set on the Current Situation page flow through to the Recommendations page via the `ExistingCover.owner` field
  * Owner defaults to the Life Insured name if not explicitly set
  * Custom owners added on the Recommendations page are available across all cover owner dropdowns
  * Add Owner Modal (same as Current Situation) allows creating custom owner names
...
* Description — Cover definition
* Premium Structure — Resolved from structure code
* Cover Structure — Derived from cover structure field or resolved cover linkage
* Super — Read-only checkbox (checked if isSuper is true)
* Owner — From cover owner field
* Life Insured — Name of life insured
* Benefit Amount — Sum insured formatted as currency
* Benefit Period — Formatted from resolved cover data
* Waiting Period — Formatted from resolved cover data
...
* **Note:** Occupation Type row is **not** shown for TPD or IP covers. It is only shown for other cover types when occupation type data exists.

* **Field Derivation Logic:**
  * Cover Type: Human-readable cover type label (e.g. "Life", "TPD", "Income Protection") — derived via `getCoverType()` from the cover's need code
  * Cover Structure: From the cover's `coverStructure` field if set; otherwise "Linked" if the resolved cover is a linked extension, "Standalone" otherwise
  * Owner: From the cover's `owner` field (set via the Recommendations page owner dropdown)
  * Premium Structure: From the cover's `premiumStructure` field if set; otherwise resolved from structure code (S = Variable age-stepped, B = Blended, L = Variable to age 65, 70 = Variable to age 70)
  * Is Super: "True" if the cover's `isSuper` flag is true; "False" otherwise
  * Benefit Amount: Formatted as currency with 2 decimal places
  * Definition: From the cover's `definition` field
...
* A new "Vary to Existing" item is added to the review table with cover structure derived using the same cover-type-specific rules as existing covers (see story 6.1 Cover Structure Derivation)
...
* Features within each group have:
  ...
  * Sub-feature value items — **each compared value and recommended value is independently selectable:**
    * "Existing: [Existing Insurer] — [compared value]" line with its own checkbox (only shown if compared value is non-blank)
    * "New: [Recommended Insurer] — [recommended value]" line with its own checkbox (only shown if recommended value is non-blank)
  * Each value checkbox can be independently toggled to include or exclude that specific value
  * Checkbox keys use the format `featureCode::subFeatureCode::compared` and `featureCode::subFeatureCode::recommended`
...
* **Sub-feature filtering:** Sub-feature values are only shown when the corresponding value (comparedValue or recommendedValue) is non-blank
